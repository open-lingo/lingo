...
- **SRS writes happen ONLY in review lessons** (`ja-mN-review-1/2`). Content sub-lessons are pure introduction. Kana (M1/M2) has no SRS at all. Enforced by the `shouldWriteSrs(step)` gate in `lesson/data/_stepPredicates.ts` plus the lesson-id check in `LessonPage.handleStepComplete`.
