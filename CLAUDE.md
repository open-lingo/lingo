...
- **⚠️ JA course is MID-REWRITE.** The **live** JA learn map is the draft-4 spine
  (`shared/domain/mockCourse.ts` ← `features/lesson/dev/spinePlan.ts`), not the old M1–M27 course:
  **m3–m7 are authored (`ja-m*-neo-*`); m8–m29 are `comingSoon` placeholders with zero lessons.**
  A learner past m7 hits "content not yet authored." **The old m3–m29 course is ARCHIVED**
