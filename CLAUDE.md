...
- **SRS writes happen in review lessons AND on the flashcard reviewer** — NOT in content sub-lessons (those are pure introduction). ⚠️ **"kana M1/M2 has no SRS" = the kana _glyphs_ only** (single-kana-no-emoji atoms → alphabet-trainer surface, excluded by `isSrsEligibleAtom`); M1/M2's vocab _words_ (あい/いえ/あおい …) **ARE** SRS-eligible, sit in the course deck (`flashcards/data/courseDeck.ts`), and **unlock on lesson completion** — they surface as **new cards** in the reviewer and just need a review action (review lesson or the reviewer) to enter FSRS. Mind the ladder: **unlocked ≠ has FSRS state ≠ due** (the derived "reviews due" chip counts only cards with state that are due, so a fresh content-only learner shows 0 due but already has unlocked new cards waiting). In-lesson writes are gated to `ja-mN-review-1/2` by `shouldWriteSrs(step)` (`lesson/data/_stepPredicates.ts`) + the lesson-id check in `LessonPage.handleStepComplete`; the flashcard reviewer (`FlashcardTester` → `reviewCard` → `setCardState`) is an independent on-ramp. Two FSRS tracks now exist: **Track A** vocab atoms (`open-lingo-srs:v2`) and **Track B** grammar points (`open-lingo-srs-grammar:v1`, `engine/grammarSrs.ts`) — review lessons advance both via `exercisedAtoms` + `exercisedGrammar`. See `docs/retention-architecture-design-2026-06-13.md`.
