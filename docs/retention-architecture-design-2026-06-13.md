...
- **D6 — Backend scope.** Rec: **piggyback the local-first SRS store with a new key namespace** (`open-lingo-srs-grammar:v1`) mirroring `srsSync.ts` delta-merge, so Track B doesn't block on backend.
