...
| Kana→kanji switchover beat (B061) | ⚠️ **the beat is DORMANT — its only call site is `buildSrsReviewLesson`, which nothing on the live map routes to (B069, blocker, verified twice 07-29)**. `kanji-switchover-design-2026-07-28.md` — **§6 is the 07-29 decision** (variant C, 2 steps: animated reveal + graded sentence question); §1 the measured scope, §2 the research. Companion: `kanji-switchover-distributed-spec-2026-07-28.md` (a REJECTED 3-beat plan kept for its findings — §6a proves review tails don't exist at m8+, §7a is the pretesting evidence the decision overrules). Live: `/ja/qa/kanji-switchover` (variant gallery) → `/ja/qa/kanji-reveal` (animation bake-off, 6 candidates). Stroke data: `shared/glyphs/data/kanji.json`, 147 glyphs, rebuild with `node scripts/build-kanjivg-data.mjs` |
| **Taught words that never unlock (B068 — fix option 2 SHIPPED 2026-07-29, 287→226)** | `vocab-exposure-audit-2026-07-29.md` **§5.1** — atoms graded by a live lesson that never enter the unlock store (unlock gates the flashcard deck + every dynamic review pool). Was 287; `isDeadAttribution` in `lessonAtomIndex.ts` now treats an unresolvable `introducedByLessonId` as unset so the module fallback fires (positive-controlled). Residual 226 = legacy-`fromModule` mismatch, cleared per vocab pack by re-homing. Ratcheted in `lessonAtomAttribution.test.ts`: MAX_DANGLING_ATTRIBUTIONS 234, MAX_GRADED_NEVER_UNLOCKABLE 226 — lower with each pack. `applyPlacement.ts` ~147 remains the placement-only second unlock path |
| Vocab pack wave — the PLAN (approved direction) | `vocab-pack-wave-plan-2026-07-29.md` — 16 mixed-POS in-course pack lessons (draft tables), then Travel Sprint + Anime quest (blocked by B066), QA gates incl. Sonnet learner-sim; word lists locked per-module at authoring time |
| Open product decisions awaiting Spencer | `decision-brief-2026-07-29.md` — B069 wiring (rec: render-time prepend, option 3), retired -kata rows keep/drop, B040 giving-verbs go/no-go + module home, dialogue_sim's 5 design rulings (prototype live at /ja/qa/dialogue-sim), MAX_NEW raise |
