...
> current picture of those, see `docs/retrospective-2026-07-17.md`, `CLAUDE.md`, and the
> authoring guide §4d–§4g. Refresh this file before trusting it as the source of truth.

> **⚠️ JA COURSE IS MID-REWRITE (as of 2026-07-20).** The route + module tables below describe a
> full M1–M27 JA course that is **no longer the live learner-facing map.** The JA learn map was
> replaced by the **draft-3 spine big-bang** (`shared/domain/mockCourse.ts` ← `spinePlan.ts`
> `SPINE_VERSION="draft-3"`): **only m3 (`ja-m3-neo-*`, 7 lessons) is authored; m4–m29 are
> `comingSoon` placeholder modules with zero lessons.** The old authored m3–m28 content is
> **off-map — deep-link only via `/ja/qa`**. A learner past m3 currently hits "content not yet
> authored." Do not trust the JA course/route tables here until the spine reauthor lands. See
> `docs/plan-code-reconciliation-2026-07-20.md` §2.1.
...
### Recent (2026-07-30/31 — practice revamp + dictionary + curated content + per-language romanization)

> Additive note; the rest of this file was NOT re-verified and remains stale (see banners above).

- **Practice pillars revamped.** Speaking/Writing/Listening + reading-cloze generate from the learner's own learned vocab via the practice engine (`features/practice/engine/`: `learnedContent` + templates + `generatePracticeItems`). Shipped `practice-tailored`.
- **Comprehension moved to curated authored content.** Random generation produced nonsense ("I can't drink a leg") for Reading/Listening, so those surfaces now use **authored, module-gated, comprehensibility-gated Stories + Conversations** (`features/practice/content/{ja,ko}.ts`, `gate.ts`). Reading = **Stories | Fill-in-the-blank** top-level tabs; story-picker list → preview modal (difficulty tier + module words) → reader. Two conversation modes: comprehension **listener** + interactive **roleplay**. The generator is kept only for Speaking/Writing + cloze over authored sentences. Story density ramps (sparse early → 1–2/module late). Shipped `curated-content`.
- **Inline dictionary lookup.** Language-agnostic dictionary service (`shared/dictionary/`: `lookupWord`/`searchDictionary`/`foldText`) + `<TappableText>` (longest-match tokenization → dictionary modal), reused across reading/listening/stories/conversations. Plus a vocab viewer UI and optional frequency-ranked vocab. Shipped `vocab-viewer` + `freq-vocab`.
- **Romanization is now per-language.** `settings.learning.showRomanization` went from a single global boolean → `Record<languageId, boolean>` behind one language-agnostic toggle surface, so JA-on + KO-off is possible. Read via `isRomanizationOn(learning, languageId)` (absent key = on); JA-specific "romaji" internals (per-script fade, build-tile, kana conversion) keep the `romaji` name. Legacy keys migrated on hydrate in `SettingsContext`. Shipped `romaji-rename` + `romanization-per-language`.
...
- **Third language shipped: Spanish (es), full CEFR A1 arc** — 16 modules × 8 lessons (128 lessons, ~1,100 steps), 434 atoms (gendered nouns), placement bank (16 screeners + 64 items), 10-verb × 18-form conjugation tables, 8 reading passages, 24 speaking prompts, ~1,200 es-MX TTS clips (Dalia voice). LatAm-neutral (ustedes; vosotros culture-noted). Authored by 8 parallel agents against `docs/archive/es-course-spine-2026-07-13.md` (pre-allocated vocab per module = zero merge conflicts); ko house style (base Atom, infoStep grammar, no grammar_rule yet).
...
- **SRS scheduling model spec** — `docs/srs-scheduling-model-2026-06-15.md` (D1–D8), ralph-hardened; **supersedes** the intake decisions in `archive/retention-architecture-design-2026-06-13.md`.
