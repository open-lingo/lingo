...
| T35 | PT phase C: five authoring lanes (m1 L1–L5, fragments `ir/m1/l{n}.ir.yaml` + `courseAtoms.m1-l{n}.ts`), then one TTS lane (pt-BR-FranciscaNeural), then PTQA (flashcards deck for pt renders with art + audio, `/pt/qa/m1` walk, sim capture), then build 35 with the allow-list (Spencer + Cris) | lanes PTAUTH-L1..5 | in progress |
| T36 | Spec-first PT authoring toolkit (`scripts/author/pt/`: pack.mjs → docs/pt-authoring-pack.md, from-spec.mjs, check.sh) — measured: 62 % of a lane's calls were orientation, 21 % gate loops; target 1–2 min / ≤15 calls per lesson; then re-author L1–L5 with it and compare | lane PTTOOL → re-author lanes | in progress |
| T37 | PT TTS follow-ups: pt LANG_CONFIG row + male NPC voice (AntonioNeural) in lingo-data's generate.py; per-language `pt/__tests__/emitTtsDeck.test.ts` (the JA deck emitter is kanji-specific) | next TTS lane | queued |
| T38 | PT m1 needs an L6 checkpoint lesson (zero new atoms, recall of all 40, ends on sim) before the module compiles; `content:emit` must include `pt`; `LangLayout` redirects `/pt/*` on cold load before feature flags resolve (fix before Cris deep-links); then flashcards deck check + sim captures → build 35 | round-1 re-author (L6) + lane PTQA | queued |
