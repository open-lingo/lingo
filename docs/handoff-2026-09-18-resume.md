...
| T29 | Test-suite audit: 19,132 tests / 684 s CPU — 56 files hold 9,905 tests (per-step it.each), top 25 files = 67 % of CPU (whole-course sweeps re-deriving the course each); merge sweeps onto one memoised course walk, collapse per-step expansion, fix the isolate:false order leak (kanjiCoverageAudit), local maxWorkers 50 %, nightly project for property/slow suites; targets < 6,000 tests, top-10 CPU < 60 s, mutation score unchanged | lane TESTAUDIT | in progress |
| T30 | `SRSCardState.known` never syncs (no server field; pull overwrites) — the due-count mismatch (phone 151 vs server 354) | lane SRSGAPS (A) | in progress |
| T31 | Test-out seeds vocab SRS only; grammar SRS (Track B) never seeded → placed learners face a full grammar review backlog | lane SRSGAPS (B) | in progress |
| T32 | Home/Learn/Practice cards refresh once per completed sync via a single progressChanged signal (Continue button was stale after the build-33 bulk sync); inventory every progress reader + writer, no extra requests | lane HOMEREFRESH | in progress |
