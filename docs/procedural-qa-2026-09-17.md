...
| Q11 | does the English gloss carry the aspect the target form asserts (rather than relying on a subtle English grammar contrast the learner won't notice)? | `scripts/gloss-aspect/` — `extract.mjs` (source IR → rows tagged with the house-gloss-table form, `forms.mjs`) → `judge.mjs` (local judge, rationale-first, `gemma4-31b-256k`) → `<lang>-findings.json`. Evidence + rule: `docs/learning-loop-2026-09-17.md` §5, `docs/lesson-authoring-guide.md` §7b, `docs/authoring-invariants-pinned.md` #51. | **No** — informational, JA only for now (added 2026-09-18, lane GLOSS; precision measured on a 30-row hand audit, not yet promoted — do NOT add to `src/test/proceduralQa.test.ts` until it is; not wired into `scripts/qa/procedural/checks/` — this is a standalone offline sweep, run via `npm run gloss-aspect -- ja`, not part of the per-lesson `run.mjs` walk) |
