...
// 6=m19-23, 7=m24-27, 8=m28-29, 9=m30-31, 10=m32-33, 11=m34-35, 12=m36-37,
// 13=m38-39, 14=m40-41, 15=m42-43, 16=m44-46 (see tiers.ts). Bands group
// those into learner-facing self-declarations. M1/M2 are hiragana/katakana
// script (auto-completed on any grammar pass), so bands start at M3.
//
...
//
// 2026-09-14 (TestFlight #80 QA, defect 2): the SAME bug recurred — tiers
// 9-16 (m30-m46, the N4 line) were added to tiers.ts but the n5 band below
// still capped credit at m29, so a learner who placed/tested out through
// the whole N4 line still couldn't be credited past N5. Fixed by adding a
// new top band ("n4") whose bandModules run through m46.
const JA_LEVEL_BANDS: readonly PlacementLevelBand[] = [
  ...
  },
  {
    ...
    bandModules: [
      ...
      // with tier 8. Its LAST entry used to be the credit cap (see the n4
      // band added below, 2026-09-14) — it now just marks the N5/N4
      // boundary, so a learner who declares only this far is bounded at m29.
      "m28",
      "m29",
    ],
  },
  {
    id: "n4",
    label: "I've finished N5 / I'm working on JLPT N4",
    description: "You know て-helpers, conditionals, passive, and causative.",
    // This band is now the top one, so its last entry (m46) is the highest
    // module the whole placement flow can credit — see the ⚠️ note above.
    bandModules: [
      "m30",
      "m31",
      "m32",
