...
export function BuildSentenceStepView({ step, onComplete, onContinue, isReplayRun = false, lessonId }: Props) {
  ...
  // ones — the density gate that used to guard this call is gone. GHOST
  // follow-up (2026-09-18): the hook itself moved to
  // `hooks/useSpentTileCollapse.ts` and lost the "huge bank" name — see its
  // own doc comment for why the fade is safe on every bank size AND every
  // build-type surface (the tray no longer grows, so no row is ever "given
