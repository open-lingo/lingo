...
export function jaVariantSurfaces(
  ...
  // JA author-listed alternates ride this lane too, not lane 3: JA tiles
  // split below the word (あさ|が), so the space-preserving lane-3 compare
  // can never match an alternate authored as あさが. Here spacing is
  // stripped and each alternate gets the same variant expansion as the
  // target (TestFlight #21; BuildSentenceLeniency.test.tsx).
  const seeds = [seed, ...(step.alsoAccepted ?? [])];
  return new Set(
    expandAcceptedAnswers(seeds, { moduleIndex }).map((v) => normalizeTypedAnswer(v)),
  );
}
