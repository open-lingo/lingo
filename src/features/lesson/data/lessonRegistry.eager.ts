...
import { registerLessons } from "./lessonRegistry";

/**
 * EAGER lesson registry bootstrap — tests and the content emitter only.
 *
 * This is the static import block that used to sit at the top of
 * `mockLessons.ts` (content-as-data split, 2026-09-13). It registers every
 * course synchronously at import so `getAvailableMockLessonIds()` and every
 * curriculum gate see the full table without touching the network. The
 * browser never imports this file: `mockLessons.ts` reaches it only through
 * `virtual:lesson-registry-bootstrap`, which resolves to an empty module in
 * real builds (see the plugin in vite.config.ts) and to this file under
 * vitest / `CONTENT_EMIT`. Runtime content comes from `contentLoader.ts`.
 *
 * Keep this file free of behaviour: imports, the table, one register call.
 */
...
import { M39_NEO_LESSONS } from "@/features/languages/ja/curriculum/m39-neo";
import { M40_NEO_LESSONS } from "@/features/languages/ja/curriculum/m40-neo";
import { M41_NEO_LESSONS } from "@/features/languages/ja/curriculum/m41-neo";
import { M42_NEO_LESSONS } from "@/features/languages/ja/curriculum/m42-neo";
import { M43_NEO_LESSONS } from "@/features/languages/ja/curriculum/m43-neo";
import { M44_NEO_LESSONS } from "@/features/languages/ja/curriculum/m44-neo";
import { M45_NEO_LESSONS } from "@/features/languages/ja/curriculum/m45-neo";
import { M46_NEO_LESSONS } from "@/features/languages/ja/curriculum/m46-neo";
...
const LESSONS: Record<string, LessonContent> = {
  ...
  ...Object.fromEntries(M39_NEO_LESSONS.map((l) => [l.id, l] as const)),
  ...Object.fromEntries(M40_NEO_LESSONS.map((l) => [l.id, l] as const)),
  ...Object.fromEntries(M41_NEO_LESSONS.map((l) => [l.id, l] as const)),
  ...Object.fromEntries(M42_NEO_LESSONS.map((l) => [l.id, l] as const)),
  ...Object.fromEntries(M43_NEO_LESSONS.map((l) => [l.id, l] as const)),
  ...Object.fromEntries(M44_NEO_LESSONS.map((l) => [l.id, l] as const)),
  ...Object.fromEntries(M45_NEO_LESSONS.map((l) => [l.id, l] as const)),
  ...Object.fromEntries(M46_NEO_LESSONS.map((l) => [l.id, l] as const)),
...
/** The full static table (emitter + tests). */
export const EAGER_LESSONS: Readonly<Record<string, LessonContent>> = LESSONS;
