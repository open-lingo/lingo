/**
 * JA M19 curriculum guard — Gate 2 of docs/retrospective-2026-07-17.md §4
 ...
 * this file adds M19-specific spot assertions.
 */
...
describe("JA M19 module-specific content", () => {
  it("teaches its headline grammar points via grammar_rule cards", () => {
    const gps = moduleGrammarPointIds("m19");
    expect(gps).toContain("family-register");
    expect(gps).toContain("counter-sai");
  });

  it("ships its full lesson set including the story lesson", () => {
    const ids = getJaModuleLessons("m19").map((l) => l.id);
    expect(ids.length).toBeGreaterThanOrEqual(15);
    expect(ids).toContain("ja-m19-story");
  });
});
