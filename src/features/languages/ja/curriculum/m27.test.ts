/**
 * JA M27 curriculum guard — Gate 2 of docs/retrospective-2026-07-17.md §4
 ...
 * this file adds M27-specific spot assertions.
 */
...
describe("JA M27 module-specific content", () => {
  it("teaches its headline grammar points via grammar_rule cards", () => {
    const gps = moduleGrammarPointIds("m27");
    expect(gps).toContain("nakereba-naranai");
    expect(gps).toContain("hou-ga-ii");
  });

  it("ships its full lesson set including the story lesson", () => {
    const ids = getJaModuleLessons("m27").map((l) => l.id);
    expect(ids.length).toBeGreaterThanOrEqual(15);
    expect(ids).toContain("ja-m27-story");
  });
});
