/**
 * JA M13 curriculum guard — Gate 2 of docs/retrospective-2026-07-17.md §4
 ...
 * this file adds M13-specific spot assertions.
 */
...
describe("JA M13 module-specific content", () => {
  it("teaches its headline grammar points via grammar_rule cards", () => {
    const gps = moduleGrammarPointIds("m13");
    expect(gps).toContain("kara-because");
    expect(gps).toContain("frequency-adverbs");
  });

  it("ships its full lesson set including the story lesson", () => {
    const ids = getJaModuleLessons("m13").map((l) => l.id);
    expect(ids.length).toBeGreaterThanOrEqual(15);
    expect(ids).toContain("ja-m13-story");
  });
});
