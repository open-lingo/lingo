/**
 * JA M8 curriculum guard — Gate 2 of docs/retrospective-2026-07-17.md §4
 ...
 * this file adds M8-specific spot assertions.
 */
...
describe("JA M8 module-specific content", () => {
  it("teaches its headline grammar points via grammar_rule cards", () => {
    const gps = moduleGrammarPointIds("m8");
    expect(gps).toContain("i-adj-present");
    expect(gps).toContain("to-and");
  });

  it("ships its full lesson set including the story lesson", () => {
    const ids = getJaModuleLessons("m8").map((l) => l.id);
    expect(ids.length).toBeGreaterThanOrEqual(18);
    expect(ids).toContain("ja-m8-story");
  });
});
