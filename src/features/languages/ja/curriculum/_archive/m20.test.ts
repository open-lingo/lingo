/**
 * JA M20 curriculum guard — Gate 2 of docs/retrospective-2026-07-17.md §4
 ...
 * this file adds M20-specific spot assertions.
 */
...
describe("JA M20 module-specific content", () => {
  it("teaches its headline grammar points via grammar_rule cards", () => {
    const gps = moduleGrammarPointIds("m20");
    expect(gps).toContain("ga-itai");
    expect(gps).toContain("node-because");
  });

  it("ships its full lesson set including the story lesson", () => {
    const ids = getJaModuleLessons("m20").map((l) => l.id);
    expect(ids.length).toBeGreaterThanOrEqual(15);
    expect(ids).toContain("ja-m20-story");
  });
});
