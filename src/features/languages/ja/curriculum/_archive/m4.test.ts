/**
 * JA M4 curriculum guard — Gate 2 of docs/retrospective-2026-07-17.md §4
 ...
 * this file adds M4-specific spot assertions.
 */
...
describe("JA M4 module-specific content", () => {
  it("teaches its headline grammar points via grammar_rule cards", () => {
    const gps = moduleGrammarPointIds("m4");
    expect(gps).toContain("no-possession");
    expect(gps).toContain("kore-sore-are-dore");
  });

  it("ships its full lesson set including the story lesson", () => {
    const ids = getJaModuleLessons("m4").map((l) => l.id);
    expect(ids.length).toBeGreaterThanOrEqual(16);
    expect(ids).toContain("ja-m4-story");
  });
});
