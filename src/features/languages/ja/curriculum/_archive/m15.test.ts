/**
 * JA M15 curriculum guard — Gate 2 of docs/retrospective-2026-07-17.md §4
 ...
 * this file adds M15-specific spot assertions.
 */
...
describe("JA M15 module-specific content", () => {
  it("teaches its headline grammar points via grammar_rule cards", () => {
    const gps = moduleGrammarPointIds("m15");
    expect(gps).toContain("te-iru");
    expect(gps).toContain("v-tai");
  });

  it("ships its full lesson set including the story lesson", () => {
    const ids = getJaModuleLessons("m15").map((l) => l.id);
    expect(ids.length).toBeGreaterThanOrEqual(15);
    expect(ids).toContain("ja-m15-story");
  });
});
