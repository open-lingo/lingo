/**
 * JA M26 curriculum guard — Gate 2 of docs/retrospective-2026-07-17.md §4
 ...
 * this file adds M26-specific spot assertions.
 */
...
describe("JA M26 module-specific content", () => {
  it("teaches its headline grammar points via grammar_rule cards", () => {
    const gps = moduleGrammarPointIds("m26");
    expect(gps).toContain("n-desu");
    expect(gps).toContain("sugiru");
  });

  it("ships its full lesson set including the story lesson", () => {
    const ids = getJaModuleLessons("m26").map((l) => l.id);
    expect(ids.length).toBeGreaterThanOrEqual(15);
    expect(ids).toContain("ja-m26-story");
  });
});
