/**
 * JA M6 curriculum guard — Gate 2 of docs/retrospective-2026-07-17.md §4
 ...
 * this file adds M6-specific spot assertions.
 */
...
describe("JA M6 module-specific content", () => {
  it("teaches its headline grammar points via grammar_rule cards", () => {
    const gps = moduleGrammarPointIds("m6");
    expect(gps).toContain("ni-location");
    expect(gps).toContain("de-action");
    expect(gps).toContain("ga-existence");
  });

  it("ships its full lesson set including the story lesson", () => {
    const ids = getJaModuleLessons("m6").map((l) => l.id);
    expect(ids.length).toBeGreaterThanOrEqual(18);
    expect(ids).toContain("ja-m6-story");
  });
});
