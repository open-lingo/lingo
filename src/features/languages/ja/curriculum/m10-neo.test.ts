/**
 * m10-neo module guards. First module registered against the 2026-07-26
...
registerJaModuleContentLints("m10");

/** The IR-compiled lessons — katakana rows are a different taxonomy. */
const COMPILED = M10_NEO_LESSONS.filter((l) => !l.id.includes("-kata-"));
...
describe("m10-neo module shape (invariant 25)", () => {
  it("ships 15 lessons: 11 teaching + 3 review + 1 challenge", () => {
    expect(M10_NEO_LESSONS).toHaveLength(15);
    const reviews = M10_NEO_LESSONS.filter((l) => /-review(-\d+)?$/.test(l.id));
    const challenge = M10_NEO_LESSONS.filter((l) => l.id.endsWith("-challenge"));
    expect(reviews, reviews.map((l) => l.id).join(", ")).toHaveLength(3);
    expect(challenge).toHaveLength(1);
    expect(M10_NEO_LESSONS.length - reviews.length - challenge.length).toBe(11);
  });

  it("the CHALLENGE lesson is last", () => {
    expect(M10_NEO_LESSONS[M10_NEO_LESSONS.length - 1].id).toBe("ja-m10-neo-challenge");
  });

  it("review lessons sit in the beginning/middle/end thirds", () => {
    const body = M10_NEO_LESSONS.filter((l) => !l.id.endsWith("-challenge"));
  ...
  it("carries the two katakana row lessons (ruling 2026-07-26)", () => {
    const kata = M10_NEO_LESSONS.filter((l) => l.id.includes("-kata-")).map((l) => l.id);
    expect(kata).toEqual(["ja-m10-neo-kata-ma", "ja-m10-neo-kata-ya"]);
  });

  it("every lesson is reachable by deep link", () => {
    for (const l of M10_NEO_LESSONS) {
