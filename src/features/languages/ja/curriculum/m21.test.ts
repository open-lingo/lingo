/**
 * JA M21 curriculum guard — Gate 2 of docs/retrospective-2026-07-17.md §4
 ...
 * this file adds M21-specific spot assertions.
 */
...
describe("JA M21 module-specific content", () => {
  it("teaches its headline grammar points via grammar_rule cards", () => {
    const gps = moduleGrammarPointIds("m21");
    expect(gps).toContain("to-quotation");
    expect(gps).toContain("counter-hai");
  });

  it("ships its full lesson set including the story lesson", () => {
    const ids = getJaModuleLessons("m21").map((l) => l.id);
    expect(ids.length).toBeGreaterThanOrEqual(15);
    expect(ids).toContain("ja-m21-story");
  });
});
