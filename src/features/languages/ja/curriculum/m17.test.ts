/**
 * JA M17 curriculum guard — Gate 2 of docs/retrospective-2026-07-17.md §4
 ...
 * this file adds M17-specific spot assertions.
 */
...
describe("JA M17 module-specific content", () => {
  it("teaches its headline grammar points via grammar_rule cards", () => {
    const gps = moduleGrammarPointIds("m17");
    expect(gps).toContain("e-direction");
    expect(gps).toContain("made-ni");
  });

  it("ships its full lesson set including the story lesson", () => {
    const ids = getJaModuleLessons("m17").map((l) => l.id);
    expect(ids.length).toBeGreaterThanOrEqual(17);
    expect(ids).toContain("ja-m17-story");
  });
});
