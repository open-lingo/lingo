/**
 * JA M10 curriculum guard — Gate 2 of docs/retrospective-2026-07-17.md §4
 ...
 * this file adds M10-specific spot assertions.
 */
...
describe("JA M10 module-specific content", () => {
  it("teaches its headline grammar points via grammar_rule cards", () => {
    const gps = moduleGrammarPointIds("m10");
    expect(gps).toContain("masu-past");
    expect(gps).toContain("i-adj-past");
  });

  it("ships its full lesson set including the story lesson", () => {
    const ids = getJaModuleLessons("m10").map((l) => l.id);
    expect(ids.length).toBeGreaterThanOrEqual(16);
    expect(ids).toContain("ja-m10-story");
  });
});
