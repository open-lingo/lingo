/**
 * Portuguese grammar helpers — thin language-idiomatic wrappers over the
 * generic lesson step types. Ports the KO helper set (same step type
 * names + the same slot-rotation / atom-resolution discipline) to PT.
 *
 * Atom resolution: keyed by the Portuguese surface form (the canonical
 * written form — Latin script, accents included). Resolution goes through
 * `findPtAtomBySurface` — the cycle-safe accessor over the live
 * `PT_ATOMS_BY_SURFACE` registry in `courseAtoms.ts` (curriculum files
 ...
 * emitted step objects are shape-identical to KO's; only the PT-facing
...
// generated `PT_REVIEW_POOL` snapshot is import-order independent, so gloss /
// id resolution falls back to it. Atom ids are deterministically `pt:<surface>`.
const POOL_BY_SURFACE = new Map(PT_REVIEW_POOL.map((e) => [e.surface, e] as const));
...
function resolveSurfaceGloss(surface: string): string | undefined {
  return findPtAtomBySurface(surface)?.gloss ?? POOL_BY_SURFACE.get(surface)?.gloss;
}

function resolveAtomId(surface: string): string | undefined {
  const live = findPtAtomBySurface(surface);
  if (live) return live.id;
  return POOL_BY_SURFACE.has(surface) ? `pt:${surface}` : undefined;
}
...
export function phrase(
  ...
  return {
    ...
    // slot) and `kana` (target-script slot) — JA-era field names. Portuguese
    // is already Latin script, so the transliteration slot stays empty
    // (spine: leave romanization unset) and `kana` carries the Portuguese
...
export function cloze(
  ...
  if (correctIdx === -1) {
    throw new Error(
      `pt cloze(${id}): correctParticle '${correctParticle}' missing from options [${options.join(", ")}]`,
    );
  }
  let rotated = options;
...
export function agreementCloze(
  ...
  if (!segments.some((seg) => "blank" in seg)) {
    throw new Error(`pt agreementCloze(${id}): segments contain no blanks`);
  }
  const rotated = segments.map((seg) => {
    ...
    if (correctIdx === -1) {
      throw new Error(
        `pt agreementCloze(${id}): blank '${blankId}' correctAnswer '${correctAnswer}' missing from options [${options.join(", ")}]`,
      );
    }
...
/**
 * `match_pairs` factory — Portuguese surface ↔ English gloss, resolved from
...
export function matchPairs(
  ...
  if (surfaces.length < 6) {
    throw new Error(
      `pt matchPairs(${idPrefix}): needs >= 6 surfaces (got ${surfaces.length})`,
    );
  }
  const pairs = surfaces.map((s, i) => {
    ...
    if (!gloss) {
      throw new Error(
        `pt matchPairs(${idPrefix}): surface '${s}' is not a registered atom`,
      );
    }
  ...
  return {
    id: `${idPrefix}-match`,
    type: "match_pairs",
    prompt: "Match each Portuguese word to its meaning",
 ...
 * contract (`pt:<surface>`) without a lookup. Keep glosses byte-identical
...
export function capstoneMatchPairs(
  ...
  if (entries.length < 6) {
    throw new Error(
      `pt capstoneMatchPairs(${idPrefix}): needs >= 6 entries (got ${entries.length})`,
    );
  }
  return {
    id: `${idPrefix}-match`,
    ...
    exercisedAtoms: entries.map((e) => `pt:${e.surface}`),
    modality: "recognition",
  };
}
 ...
 *
 * The shared step type's `kana` slot carries each line's Portuguese text —
...
export function dialogueListen(opts: {
  ...
  if (opts.lines.length < 1 || opts.lines.length > 8) {
    throw new Error(
      `pt dialogueListen(${opts.id}): lines.length must be 1-8 (got ${opts.lines.length})`,
    );
  }
  if (opts.lines.some((l) => !l.speaker)) {
    throw new Error(
      `pt dialogueListen(${opts.id}): every line must have a speaker`,
    );
  }
  if (opts.questions.length < 1 || opts.questions.length > 3) {
    throw new Error(
      `pt dialogueListen(${opts.id}): questions.length must be 1-3 (got ${opts.questions.length})`,
    );
  }
...
export function speaking(
  ...
  translation: string,
  exercisedAtomSurfaces?: string[],
  /** "recall" = cued recall: English cue shown, Portuguese hidden until the
  ...
  return {
    ...
    targetPhrase,
    translation,
    // Whisper supports Portuguese; graded path enabled from day one (same
...
/**
 * Vocab-by-image MCQ — given the meaning, pick the Portuguese tile whose
...
export function vocabMcq(
  ...
  if (!target.emoji) {
    throw new Error(
      `pt vocabMcq: target '${target.surface}' has no emoji — use listeningBuild or listeningComp instead`,
    );
  }
  ...
  if (filtered.length < 3) {
    throw new Error(
      `pt vocabMcq: distractor pool for '${target.surface}' has only ${filtered.length} emoji-bearing candidates (need 3)`,
    );
  }
 ...
 * plain Portuguese surfaces, slot-rotated by id. Use when the word has no
...
export function vocabTextMcq(
  ...
  if (!gloss) {
    throw new Error(
      `pt vocabTextMcq(${id}): target '${targetSurface}' is not a registered atom`,
    );
  }
  const distractors = distractorSurfaces.filter((s) => s !== targetSurface);
  if (distractors.length < 3) {
    throw new Error(
      `pt vocabTextMcq(${id}): needs >= 3 distractors distinct from the target (got ${distractors.length})`,
    );
  }
  const items = [
...
export function selfExplain(opts: {
  id: string;
  /** The just-committed fact, e.g. "You wrote: a casa é bonita". */
  anchorLabel: string;
...
// PT only has m1 so far (empty). extend as modules ship — mirrors
// `PT_MODULE_META` in `curriculum/index.ts`.
export const PT_MODULE_ORDER: PtAtomSource[] = ["m1"];

function moduleIndex(m: PtAtomSource): number {
  const i = PT_MODULE_ORDER.indexOf(m);
  ...
  // cause (ES's incident — the class of bug PT inherits the guard against).
  // A new module must be added here, and forgetting must say so.
  if (i === -1) {
    throw new Error(
      `pt moduleIndex: "${m}" is not in PT_MODULE_ORDER — add it when you add the module`,
    );
  }
  return i;
}
 ...
 *
 * Reads the STATIC `PT_REVIEW_POOL` (a generated snapshot), NOT the live
...
function pickReviewEntries(
  seedId: string,
  beforeModule: PtAtomSource,
  n: number,
  opts?: { singleWord?: boolean; kinds?: PtAtomKind[]; includePhrases?: boolean },
): ReviewEntry[] {
  const cutoff = moduleIndex(beforeModule);
  const singleWord = opts?.singleWord ?? true;
  const kinds = opts?.kinds ?? (opts?.includePhrases ? undefined : (["vocab", "particle"] as PtAtomKind[]));
  const pool = PT_REVIEW_POOL.filter((a) => {
    if (moduleIndex(a.fromModule as PtAtomSource) >= cutoff) return false;
    if (singleWord && a.surface.includes(" ")) return false;
    if (kinds && !kinds.includes(a.kind as PtAtomKind)) return false;
    return true;
  });
