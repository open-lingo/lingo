/**
 * FR m20 curriculum guard — the «Venir de + infinitif» recent-past module.
 * Standard suite in the m17-m19.test.ts shape, plus bespoke pins from
 * docs/fr-speech-recent-past-2026-09-10.md:
 *   (a) no `speaking` step in a TEACHING lesson is EVER the learner's
 *       first exposure to a given subject+infinitive pairing (mirrors
 *       m19's constraint 3) — every new pairing must debut written first
 *       (word_map/cloze/build/sentenceMcq/listening_comprehension) in the
 *       same lesson or earlier. The checkpoint (L8) is explicitly exempt
 *       (constraint 5 — a checkpoint may test a novel graded
 *       RECOMBINATION of two known parts, e.g. elle+parler).
 *   (b) "written-only" is lifted ONLY for L6 (the passé-composé bridge):
 *       no graded (non-recall) `speaking` step targeting the venir-de
 *       frame appears in L1/L2/L3/L4/L5/L7/L9/L10 — only L6 and the
 *       checkpoint (L8) may carry one.
 *   (c) "habiter" never appears as the fronted infinitive after
 *       "vient de"/"viens de" (vowel-onset elision gap, deliberately
 *       excluded per the module header).
 *   (d) no course sentence conflates the origin sense ("je viens de
 *       Paris") with the recent-past sense ("je viens de manger") inside
 *       a single sentence's meaning claim — the two only ever appear
 *       together as MCQ contrast options, never fused.
 */
...
import { FR_M20_ATOMS, FR_M20_MODULE, FR_M20_CHECKPOINT_INDEX } from "./m20";
...
const LESSONS = FR_M20_MODULE.lessons;
const COUNT = LESSONS.length;

// No new homophoneKey pairs this module — the four «venir de» phrase
// atoms carry no formal homophoneKey, kept empty per m12/m17/m18/
// m19.test.ts's own precedent rather than omitted, so a future addition
// has an obvious place to land.
const HOMOPHONE_PAIRS: Array<[string, string]> = [];

describe("FR m20 — §13 doctrine pins", () => {
  it("has its full lesson run and the checkpoint two before mastery", () => {
    expect(COUNT).toBe(10);
    expect(FR_M20_CHECKPOINT_INDEX).toBe(COUNT - 2);
  });
  ...
  for (let n = 1; n <= 10; n++) {
    it(`L${n}: unique ids, no adjacent same-type, match in closing zone`, () => {
      ...
      for (let i = 1; i < steps.length; i++) {
        expect(
          steps[i].type,
          `m20 L${n} adjacent same-type at ${steps[i - 1].id} / ${steps[i].id}`,
        ).not.toBe(steps[i - 1].type);
      }
      if (n !== COUNT) {
        expect(
          steps.slice(-3).map((s) => s.type),
          `m20 L${n} has no match in its closing zone`,
  ...
  it("checkpoint and mastery are graded steps only", () => {
    for (const n of [FR_M20_CHECKPOINT_INDEX, COUNT]) {
      const steps = LESSONS[n - 1].steps;
      expect(
        steps.every((s) => isGradedStep(s)),
        `m20 L${n} carries a non-graded step`,
      ).toBe(true);
    }
  });
      ...
      for (const s of l.steps) {
        ...
        for (const turn of s.turns) {
          ...
          for (const opt of turn.reply.options) {
            ...
            expect(
              accepted,
              `m20 ${l.id} ${s.id}/${turn.id}: option "${opt.text}" mirrors the NPC line but is marked wrong`,
            ).toBe(true);
          }
        }
      }
      ...
      for (const s of l.steps) {
        ...
        for (const turn of s.turns) {
          expect(
            ...
            `m20 ${l.id} ${s.id}/${turn.id} goal too wordy`,
  ...
  it("cued recall NEVER precedes a printed first voicing — across m1→m20 (R3/§13.9)", () => {
          ...
          if (s.cue === "recall") {
            if (counting) {
              ...
              expect(
                ...
                `m20 ${l.id} ${s.id}: recall of «${s.targetPhrase}» before any printed voicing`,
              ).toBe(true);
            }
          } else {
    ...
    walk(LESSONS, true);
    expect(m20Recalls).toBeGreaterThanOrEqual(8);
  });

  // ── Bespoke m20 pins (docs/fr-m20-brief-2026-09-10.md +
  //    docs/fr-speech-recent-past-2026-09-10.md) ──────────────────────────
  ...
  it('"habiter" never appears as the fronted infinitive after vient de/viens de (vowel-onset elision gap)', () => {
    const bad: string[] = [];
    for (const l of LESSONS) {
      for (const s of l.steps) {
        for (const text of allFrenchStrings(s)) {
          if (/vi(ens|ent)\s+de\s+habiter/i.test(text)) bad.push(`${l.id}/${s.id}: "${text}"`);
        }
      }
    }
    expect(bad, `"vient/viens de habiter" found: ${bad.join("; ")}`).toEqual([]);
  });

  // ── "Written-only until L6" speaking-promotion pin ──────────────────────

  const FRAME_RE = /\b(je|tu|il|elle|on)\s+(?:ne\s+)?vi(?:ens|ent)\s+(?:pas\s+)?de\s+(?:manger|parler|visiter)\b/i;

  it("no graded (non-recall) `speaking` step targets the venir-de frame before L6 (constraint 5)", () => {
    const bad: string[] = [];
    for (let n = 1; n <= 5; n++) {
      const l = LESSONS[n - 1];
      for (const s of l.steps) {
        if (s.type !== "speaking") continue;
        if (s.cue === "recall") continue;
        if (FRAME_RE.test(s.targetPhrase)) {
          bad.push(`${l.id}/${s.id}: "${s.targetPhrase}"`);
        }
      }
    }
    expect(bad, `frame speaking before promotion: ${bad.join("; ")}`).toEqual([]);
  });

  it("L7/L9/L10 stay written-only for the frame too (only L6 and the checkpoint may promote)", () => {
    const bad: string[] = [];
    ...
    expect(bad, `frame speaking outside L6/L8: ${bad.join("; ")}`).toEqual([]);
  });

  it("L6 carries exactly one graded (non-recall) speaking step targeting the frame", () => {
    const l = LESSONS[5];
    const graded = l.steps.filter(
      (s) => s.type === "speaking" && s.cue !== "recall" && FRAME_RE.test(s.targetPhrase),
    );
    expect(graded.length).toBe(1);
  });

  // ── Subject+infinitive first-exposure pin (mirrors m19's constraint 3) ──

  const INFINITIVES = ["parler", "manger", "visiter"] as const;
  const SUBJECTS = ["je", "tu", "il", "elle"] as const;

  /** Extracts every (subject, infinitive) pairing present in a sentence,
   *  base subject only (negation stripped) — a negated recent-past
  ...
  function extractPairs(text: string): string[] {
    ...
    for (const subj of SUBJECTS) {
      // positive: "subj vien(s|t) de <infinitive>"
      // negative: "subj ne vien(s|t) pas de <infinitive>"
      const posRe = new RegExp(`\\b${subj}\\s+vi(?:ens|ent)\\s+de\\s+(${INFINITIVES.join("|")})\\b`);
      const negRe = new RegExp(
        `\\b${subj}\\s+ne\\s+vi(?:ens|ent)\\s+pas\\s+de\\s+(${INFINITIVES.join("|")})\\b`,
      );
  ...
  it("no `speaking` step in a TEACHING lesson is ever the learner's first exposure to a subject+infinitive pairing (checkpoint L8 exempt)", () => {
    const debuted = new Set<string>();
    const bad: string[] = [];
    for (let n = 1; n <= COUNT; n++) {
      const l = LESSONS[n - 1];
      const isCheckpoint = n === FR_M20_CHECKPOINT_INDEX;
      for (const s of l.steps) {
  ...
  it("the checkpoint's «elle vient de parler» is a deliberate novel recombination, not reused as a teaching-lesson debut elsewhere", () => {
    ...
    expect(firstLessonIndex).toBe(FR_M20_CHECKPOINT_INDEX);
  });
});
