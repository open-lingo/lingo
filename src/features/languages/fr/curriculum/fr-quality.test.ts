...
import { FR_M1_CHECKPOINT_INDEX } from "./m1";
import { FR_M2_CHECKPOINT_INDEX } from "./m2";
import { FR_M3_CHECKPOINT_INDEX } from "./m3";
import { FR_M4_CHECKPOINT_INDEX } from "./m4";
import { FR_M5_CHECKPOINT_INDEX } from "./m5";
import { FR_M6_CHECKPOINT_INDEX } from "./m6";
import { FR_M7_CHECKPOINT_INDEX } from "./m7";
import { FR_M8_CHECKPOINT_INDEX } from "./m8";
import { FR_M9_CHECKPOINT_INDEX } from "./m9";
import { FR_M10_CHECKPOINT_INDEX } from "./m10";
import { FR_M11_CHECKPOINT_INDEX } from "./m11";
import { FR_M12_CHECKPOINT_INDEX } from "./m12";
import { FR_M13_CHECKPOINT_INDEX } from "./m13";
import { FR_M14_CHECKPOINT_INDEX } from "./m14";
import { FR_M15_CHECKPOINT_INDEX } from "./m15";
import { FR_M16_CHECKPOINT_INDEX } from "./m16";
import { FR_M17_CHECKPOINT_INDEX } from "./m17";
import { FR_M18_CHECKPOINT_INDEX } from "./m18";
import { FR_M19_CHECKPOINT_INDEX } from "./m19";
import { FR_M20_CHECKPOINT_INDEX } from "./m20";
import { FR_M21_CHECKPOINT_INDEX } from "./m21";
import { FR_M22_CHECKPOINT_INDEX } from "./m22";
import { FR_M23_CHECKPOINT_INDEX } from "./m23";
import { FR_M24_CHECKPOINT_INDEX } from "./m24";
import { FR_M25_CHECKPOINT_INDEX } from "./m25";
import { FR_M26_CHECKPOINT_INDEX } from "./m26";
import { getFrCourseAtoms } from "../courseAtoms";
import { isGradedStep } from "@/features/lesson/data/_stepPredicates";

// FR has no hand-maintained order — modules come from the curriculum glob,
// so the order derives from the lessons themselves.
const MODULE_ORDER: readonly string[] = [
  ...new Set(FR_ALL_LESSONS.map((l) => /^fr-(m\d+)-/.exec(l.id)?.[1] ?? "")),
].filter(Boolean);

/** 1-based checkpoint position per module — from the module's own export. */
const CHECKPOINT_INDEX: Record<string, number> = {
  m1: FR_M1_CHECKPOINT_INDEX,
  m2: FR_M2_CHECKPOINT_INDEX,
  m3: FR_M3_CHECKPOINT_INDEX,
  m4: FR_M4_CHECKPOINT_INDEX,
  m5: FR_M5_CHECKPOINT_INDEX,
  m6: FR_M6_CHECKPOINT_INDEX,
  m7: FR_M7_CHECKPOINT_INDEX,
  m8: FR_M8_CHECKPOINT_INDEX,
  m9: FR_M9_CHECKPOINT_INDEX,
  m10: FR_M10_CHECKPOINT_INDEX,
  m11: FR_M11_CHECKPOINT_INDEX,
  m12: FR_M12_CHECKPOINT_INDEX,
  m13: FR_M13_CHECKPOINT_INDEX,
  m14: FR_M14_CHECKPOINT_INDEX,
  m15: FR_M15_CHECKPOINT_INDEX,
  m16: FR_M16_CHECKPOINT_INDEX,
  m17: FR_M17_CHECKPOINT_INDEX,
  m18: FR_M18_CHECKPOINT_INDEX,
  m19: FR_M19_CHECKPOINT_INDEX,
  m20: FR_M20_CHECKPOINT_INDEX,
  m21: FR_M21_CHECKPOINT_INDEX,
  m22: FR_M22_CHECKPOINT_INDEX,
  m23: FR_M23_CHECKPOINT_INDEX,
  m24: FR_M24_CHECKPOINT_INDEX,
  m25: FR_M25_CHECKPOINT_INDEX,
  m26: FR_M26_CHECKPOINT_INDEX,
};
...
function moduleIndex(m: string): number {
  ...
  if (i === -1) {
    throw new Error(`fr-quality moduleIndex: "${m}" is not in the fr module order`);
  }
  return i;
}
function lessonCountOf(mod: string): number {
  return FR_ALL_LESSONS.filter((l) => moduleOf(l.id) === mod).length;
}
...
const TEACHING = FR_ALL_LESSONS.filter((l) => !isCheckpoint(l) && !isMastery(l));
const CHECKPOINTS = FR_ALL_LESSONS.filter(isCheckpoint);
const MASTERY = FR_ALL_LESSONS.filter(isMastery);
...
function stepFrenchStrings(step: LessonStep): string[] {
 ...
 *  token-for-token inside the step's French text. */
function referencesPriorModule(steps: readonly LessonStep[], currentModule: string): boolean {
  ...
  for (const step of steps) {
    ...
    for (const text of stepFrenchStrings(step)) {
...
describe("FR quality — production", () => {
  it("each teaching lesson has ≥2 generation steps, ≥1 spoken; zero typed translate anywhere", () => {
    ...
    const translate = FR_ALL_LESSONS.flatMap((l) =>
