 ...
 * pins specific to this module's speech-safety design (see m24.ts header):
 *   (a) fermé/fermée share the SAME homophoneKey (decision 3) — the working
 *       heure/heures-shaped wiring, not m12's inert euro/euros shape.
 *   (b) exactly 10 new atoms this module (decision 1).
 *   (c) NO `speaking` targetPhrase anywhere in the module contains "fermé"
 *       or "fermée" (decision 4 — the homophone is drilled only in written
 *       steps).
 */
...
import { FR_M24_ATOMS, FR_M24_MODULE, FR_M24_CHECKPOINT_INDEX } from "./m24";
...
const LESSONS = FR_M24_MODULE.lessons;
const COUNT = LESSONS.length;

// fermé/fermée share a homophoneKey (decision 3) — a deliberate, correctly
// wired homophone, so it is EXCLUDED from the co-tiling ban rather than
// listed as a risk (it is never co-tiled in an EAR-answered bank anyway —
// only in build()'s visual tile banks, which this check does not scan).
const HOMOPHONE_PAIRS: Array<[string, string]> = [];

describe("FR m24 — §13 doctrine pins", () => {
  it("has its full lesson run and the checkpoint two before mastery", () => {
    expect(COUNT).toBe(10);
    expect(FR_M24_CHECKPOINT_INDEX).toBe(COUNT - 2);
  });
  ...
  for (let n = 1; n <= 10; n++) {
    it(`L${n}: unique ids, no adjacent same-type, match in closing zone`, () => {
      ...
      for (let i = 1; i < steps.length; i++) {
        expect(
          steps[i].type,
          `m24 L${n} adjacent same-type at ${steps[i - 1].id} / ${steps[i].id}`,
        ).not.toBe(steps[i - 1].type);
      }
      if (n !== COUNT) {
        expect(
          steps.slice(-3).map((s) => s.type),
          `m24 L${n} has no match in its closing zone`,
  ...
  it("checkpoint and mastery are graded steps only", () => {
    for (const n of [FR_M24_CHECKPOINT_INDEX, COUNT]) {
      const steps = LESSONS[n - 1].steps;
      expect(
        steps.every((s) => isGradedStep(s)),
        `m24 L${n} carries a non-graded step`,
      ).toBe(true);
    }
  });
      ...
      for (const s of l.steps) {
        ...
        for (const turn of s.turns) {
          ...
          for (const opt of turn.reply.options) {
            ...
            expect(
              accepted,
              `m24 ${l.id} ${s.id}/${turn.id}: option "${opt.text}" mirrors the NPC line but is marked wrong`,
            ).toBe(true);
          }
        }
      }
      ...
      for (const s of l.steps) {
        ...
        for (const turn of s.turns) {
          expect(
            ...
            `m24 ${l.id} ${s.id}/${turn.id} goal too wordy`,
  ...
  it("cued recall NEVER precedes a printed first voicing — across m1→m24 (R3/§13.9)", () => {
          ...
          if (s.cue === "recall") {
            if (counting) {
              ...
              expect(
                ...
                `m24 ${l.id} ${s.id}: recall of «${s.targetPhrase}» before any printed voicing`,
              ).toBe(true);
            }
          } else {
    ...
    walk(LESSONS, true);
    expect(m24Recalls).toBeGreaterThanOrEqual(4);
  });

  // ── Bespoke m24 pins (this module's own speech-safety design, m24.ts
  //    header decisions 1/3/4) ──

  it("exactly 10 new atoms this module", () => {
    expect(FR_M24_ATOMS.map((a) => a.surface).sort()).toEqual(
      [
        "baguette",
        "crêpe",
        "fermé",
  ...
  it("`fermée` carries the SAME homophoneKey as `fermé` (decision 3: the wiring that actually functions)", () => {
    const ferme = FR_M24_ATOMS.find((a) => a.surface === "fermé");
    const fermee = FR_M24_ATOMS.find((a) => a.surface === "fermée");
    expect(ferme?.homophoneKey).toBeTruthy();
    expect(fermee?.homophoneKey).toBeTruthy();
    expect(fermee?.homophoneKey).toBe(ferme?.homophoneKey);
  });

  it("`ouvert`/`ouverte` carry NO homophoneKey — they are audibly distinct", () => {
    const ouvert = FR_M24_ATOMS.find((a) => a.surface === "ouvert");
    const ouverte = FR_M24_ATOMS.find((a) => a.surface === "ouverte");
    expect(ouvert?.homophoneKey).toBeFalsy();
    expect(ouverte?.homophoneKey).toBeFalsy();
  });

  it('no `speaking` targetPhrase anywhere in the module contains "fermé" or "fermée" (decision 4)', () => {
    const bad: string[] = [];
    for (const l of LESSONS) {
      for (const s of l.steps) {
        if (s.type !== "speaking") continue;
        if (/\bferm[ée]e?\b/i.test(s.targetPhrase)) {
