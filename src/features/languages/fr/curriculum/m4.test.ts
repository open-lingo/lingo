/**
 * FR m4 curriculum guard — second post-restart module (2026-09-01),
 * authored directly to the §13 doctrine in the m3.test.ts shape. The
 * recall-law walk runs ACROSS m1→m2→m3→m4: earlier modules' printed
 * voicings license m4's cross-module recalls.
 */
import { describe, it, expect } from "vitest";
import { FR_M1_MODULE } from "./m1";
import { FR_M2_MODULE } from "./m2";
import { FR_M3_MODULE } from "./m3";
import { FR_M4_ATOMS, FR_M4_MODULE, FR_M4_CHECKPOINT_INDEX } from "./m4";
...
const LESSONS = FR_M4_MODULE.lessons;
const COUNT = LESSONS.length;

// The m4 tint dictionary — any word_map showing one of these tokens must
// tint it exactly so; everything else stays untinted (§13.4). Elided
// surfaces («l'école», «l'hôtel») tint as their noun — the article is
// fused into the token.
...
describe("FR m4 — §13 doctrine pins", () => {
  it("has its full lesson run and the checkpoint two before mastery", () => {
    expect(COUNT).toBe(10);
    expect(FR_M4_CHECKPOINT_INDEX).toBe(COUNT - 2);
  });
  ...
  for (let n = 1; n <= 10; n++) {
    it(`L${n}: unique ids, no adjacent same-type, match in closing zone`, () => {
      ...
      for (let i = 1; i < steps.length; i++) {
        expect(
          steps[i].type,
          `m4 L${n} adjacent same-type at ${steps[i - 1].id} / ${steps[i].id}`,
        ).not.toBe(steps[i - 1].type);
      }
      expect(
        steps.slice(-3).map((s) => s.type),
        `m4 L${n} has no match in its closing zone`,
  ...
  it("checkpoint and mastery are graded steps only", () => {
    for (const n of [FR_M4_CHECKPOINT_INDEX, COUNT]) {
      const steps = LESSONS[n - 1].steps;
      expect(
        steps.every((s) => isGradedStep(s)),
        `m4 L${n} carries a non-graded step`,
      ).toBe(true);
    }
  });
  ...
  it("«il n'y a pas de» is only ever followed by a CONSONANT-initial word (inventory rule — no d'-elision taught yet)", () => {
    // The chunk ends in «de»; a vowel-initial follower would spell
    // «pas de école» — a form French does not write — and the elision
    // validator cannot see inside a multi-word tile. Walk every French
    // surface the module ships.
    const VOWEL = /^[aàâæeéèêëiîïoôœuùûühy]/i;
    const offenders: string[] = [];
    const scan = (text: string, where: string) => {
      const m = /il n'y a pas de\s+(\S+)/gi;
      for (const hit of text.matchAll(m)) {
        if (VOWEL.test(hit[1])) offenders.push(`${where}: "…pas de ${hit[1]}"`);
      }
    };
    for (const l of LESSONS) {
      scan(JSON.stringify(l.steps), l.id);
    }
      ...
      for (const s of l.steps) {
        ...
        for (const turn of s.turns) {
          ...
          for (const opt of turn.reply.options) {
            ...
            expect(
              accepted,
              `m4 ${l.id} ${s.id}/${turn.id}: option "${opt.text}" mirrors the NPC line but is marked wrong`,
            ).toBe(true);
          }
        }
      }
      ...
      for (const s of l.steps) {
        ...
        for (const turn of s.turns) {
          expect(
            ...
            `m4 ${l.id} ${s.id}/${turn.id} goal too wordy`,
  ...
  it("cued recall NEVER precedes a printed first voicing — across m1→m4 (R3/§13.9)", () => {
          ...
          if (s.cue === "recall") {
            if (counting) {
              ...
              expect(
                ...
                `m4 ${l.id} ${s.id}: recall of «${s.targetPhrase}» before any printed voicing`,
              ).toBe(true);
            }
          } else {
    ...
    walk(LESSONS, true);
    expect(m4Recalls).toBeGreaterThanOrEqual(8);
  });
});
