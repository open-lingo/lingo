/**
 * FR m9 curriculum guard — seventh post-restart module (2026-09-01),
 * authored per the playbook (docs/fr-authoring-playbook.md). Standard
 * suite in the m5–m8.test.ts shape, plus the audible-agreement pin.
 */
...
import { FR_M9_ATOMS, FR_M9_MODULE, FR_M9_CHECKPOINT_INDEX } from "./m9";
...
registerFrModuleBarGuards({
  moduleLabel: "m9",
  lessons: FR_M9_MODULE.lessons,
  priorModules: ["m1", "m2", "m3", "m4", "m5", "m6", "m7", "m8"],
});

const LESSONS = FR_M9_MODULE.lessons;
const COUNT = LESSONS.length;

// The m9 tint dictionary (§13.4). Adjectives tint with their form's side
// — the visible agreement chain is the module.
...
describe("FR m9 — §13 doctrine pins", () => {
  it("has its full lesson run and the checkpoint two before mastery", () => {
    expect(COUNT).toBe(10);
    expect(FR_M9_CHECKPOINT_INDEX).toBe(COUNT - 2);
  });
  ...
  for (let n = 1; n <= 10; n++) {
    it(`L${n}: unique ids, no adjacent same-type, match in closing zone`, () => {
      ...
      for (let i = 1; i < steps.length; i++) {
        expect(
          steps[i].type,
          `m9 L${n} adjacent same-type at ${steps[i - 1].id} / ${steps[i].id}`,
        ).not.toBe(steps[i - 1].type);
      }
      expect(
        steps.slice(-3).map((s) => s.type),
        `m9 L${n} has no match in its closing zone`,
  ...
  it("checkpoint and mastery are graded steps only", () => {
    for (const n of [FR_M9_CHECKPOINT_INDEX, COUNT]) {
      const steps = LESSONS[n - 1].steps;
      expect(
        steps.every((s) => isGradedStep(s)),
        `m9 L${n} carries a non-graded step`,
      ).toBe(true);
    }
  });
  ...
  it("only AUDIBLE agreement pairs ship — the joli/jolie written-only class waits for the homophone machinery (m9.ts header)", () => {
    const INAUDIBLE = ["joli", "jolie", "vrai", "vraie", "poli", "polie", "fatigué", "fatiguée"];
    for (const l of LESSONS) {
      const text = JSON.stringify(l.steps).toLowerCase();
      for (const w of INAUDIBLE) {
        expect(
          new RegExp(`(^|[^a-zà-ÿœ'])${w}([^a-zà-ÿœ]|$)`).test(text),
          `${l.id} ships written-only adjective "${w}"`,
        ).toBe(false);
      }
    }
  });

  it("agreement_chain steps appear (the type's FR debut) and every chain has ≥2 slots", () => {
    let chains = 0;
    for (const l of LESSONS) {
      for (const s of l.steps) {
        if (s.type !== "agreement_chain") continue;
        chains++;
        const slots = s.tokens.filter((t) => t.kind === "slot");
        expect(slots.length, `${l.id}/${s.id} chain slots`).toBeGreaterThanOrEqual(2);
      }
    }
      ...
      for (const s of l.steps) {
        ...
        for (const turn of s.turns) {
          ...
          for (const opt of turn.reply.options) {
            ...
            expect(
              accepted,
              `m9 ${l.id} ${s.id}/${turn.id}: option "${opt.text}" mirrors the NPC line but is marked wrong`,
            ).toBe(true);
          }
        }
      }
      ...
      for (const s of l.steps) {
        ...
        for (const turn of s.turns) {
          expect(
            ...
            `m9 ${l.id} ${s.id}/${turn.id} goal too wordy`,
  ...
  it("cued recall NEVER precedes a printed first voicing — across m1→m9 (R3/§13.9)", () => {
          ...
          if (s.cue === "recall") {
            if (counting) {
              ...
              expect(
                ...
                `m9 ${l.id} ${s.id}: recall of «${s.targetPhrase}» before any printed voicing`,
              ).toBe(true);
            }
          } else {
    ...
    walk(LESSONS, true);
    expect(m9Recalls).toBeGreaterThanOrEqual(8);
  });
});
