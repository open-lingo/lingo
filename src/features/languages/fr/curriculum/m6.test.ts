/**
 * FR m6 curriculum guard — fourth post-restart module (2026-09-01),
 * authored per the playbook (docs/fr-authoring-playbook.md). Standard
 * suite in the m5.test.ts shape, plus the partitive-safety inventory pin.
 */
...
import { FR_M6_ATOMS, FR_M6_MODULE, FR_M6_CHECKPOINT_INDEX } from "./m6";
...
const LESSONS = FR_M6_MODULE.lessons;
const COUNT = LESSONS.length;

// The m6 tint dictionary (§13.4).
...
describe("FR m6 — §13 doctrine pins", () => {
  it("has its full lesson run and the checkpoint two before mastery", () => {
    expect(COUNT).toBe(10);
    expect(FR_M6_CHECKPOINT_INDEX).toBe(COUNT - 2);
  });
  ...
  for (let n = 1; n <= 10; n++) {
    it(`L${n}: unique ids, no adjacent same-type, match in closing zone`, () => {
      ...
      for (let i = 1; i < steps.length; i++) {
        expect(
          steps[i].type,
          `m6 L${n} adjacent same-type at ${steps[i - 1].id} / ${steps[i].id}`,
        ).not.toBe(steps[i - 1].type);
      }
      expect(
        steps.slice(-3).map((s) => s.type),
        `m6 L${n} has no match in its closing zone`,
  ...
  it("checkpoint and mastery are graded steps only", () => {
    for (const n of [FR_M6_CHECKPOINT_INDEX, COUNT]) {
      const steps = LESSONS[n - 1].steps;
      expect(
        steps.every((s) => isGradedStep(s)),
        `m6 L${n} carries a non-graded step`,
      ).toBe(true);
    }
  });
  ...
  it("no partitive ever ships — du / de la / de l' are untaught (playbook §6)", () => {
    // The inventory is partitive-safe by design (countable orderables
    // only); this pin keeps it that way through future edits.
    for (const l of LESSONS) {
      const text = JSON.stringify(l.steps).toLowerCase();
      for (const p of ['"du ', " du ", "de la salade", "de l'eau", "du lait", "du fromage", "du pain"]) {
        expect(text.includes(p), `${l.id} contains partitive-shaped "${p}"`).toBe(false);
      }
    }
  });
      ...
      for (const s of l.steps) {
        ...
        for (const turn of s.turns) {
          ...
          for (const opt of turn.reply.options) {
            ...
            expect(
              accepted,
              `m6 ${l.id} ${s.id}/${turn.id}: option "${opt.text}" mirrors the NPC line but is marked wrong`,
            ).toBe(true);
          }
        }
      }
      ...
      for (const s of l.steps) {
        ...
        for (const turn of s.turns) {
          expect(
            ...
            `m6 ${l.id} ${s.id}/${turn.id} goal too wordy`,
  ...
  it("cued recall NEVER precedes a printed first voicing — across m1→m6 (R3/§13.9)", () => {
          ...
          if (s.cue === "recall") {
            if (counting) {
              ...
              expect(
                ...
                `m6 ${l.id} ${s.id}: recall of «${s.targetPhrase}» before any printed voicing`,
              ).toBe(true);
            }
          } else {
    ...
    walk(LESSONS, true);
    expect(m6Recalls).toBeGreaterThanOrEqual(8);
  });
});
