 ...
 * specific to this module's speech-safety design (see m23.ts header,
 * decision 7):
 *   (a) no `speaking` target differs from ANY other `speaking` target
 *       anywhere in the module by ONLY a numeral swap (the classic
 *       "il est une heure"/"il est deux heures" minimal pair the
 *       course-wide gate treats as HIGH RISK) — expected VACUOUS by
 *       construction (this module caps non-recall speaking targets at 3,
 *       none of which is a bare numeral-swap of another), asserted
 *       explicitly rather than assumed.
 *   (b) `heures` carries the SAME homophoneKey as `heure` (the corrected,
 *       actually-functioning wiring — see decision 1 in m23.ts's header;
 *       m12's euro/euros pair does NOT actually wire this way).
 *   (c) at most 3 graded non-recall `speaking` targets for the whole
 *       module (policy pin, adapted from m22's "exactly one").
 *   (d) zero "qui" usage anywhere in the module (this module never needs
 *       it — mirrors m22's own pin).
 */
...
import { FR_M23_ATOMS, FR_M23_MODULE, FR_M23_CHECKPOINT_INDEX } from "./m23";
...
const LESSONS = FR_M23_MODULE.lessons;
const COUNT = LESSONS.length;

// heure/heures share a homophoneKey (see decision 1) — that pair is a
// deliberate, correctly-wired homophone, so it is EXCLUDED from the
// co-tiling ban rather than listed as a risk.
const HOMOPHONE_PAIRS: Array<[string, string]> = [];

describe("FR m23 — §13 doctrine pins", () => {
  it("has its full lesson run and the checkpoint two before mastery", () => {
    expect(COUNT).toBe(10);
    expect(FR_M23_CHECKPOINT_INDEX).toBe(COUNT - 2);
  });
  ...
  for (let n = 1; n <= 10; n++) {
    it(`L${n}: unique ids, no adjacent same-type, match in closing zone`, () => {
      ...
      for (let i = 1; i < steps.length; i++) {
        expect(
          steps[i].type,
          `m23 L${n} adjacent same-type at ${steps[i - 1].id} / ${steps[i].id}`,
        ).not.toBe(steps[i - 1].type);
      }
      if (n !== COUNT) {
        expect(
          steps.slice(-3).map((s) => s.type),
          `m23 L${n} has no match in its closing zone`,
  ...
  it("checkpoint and mastery are graded steps only", () => {
    for (const n of [FR_M23_CHECKPOINT_INDEX, COUNT]) {
      const steps = LESSONS[n - 1].steps;
      expect(
        steps.every((s) => isGradedStep(s)),
        `m23 L${n} carries a non-graded step`,
      ).toBe(true);
    }
  });
      ...
      for (const s of l.steps) {
        ...
        for (const turn of s.turns) {
          ...
          for (const opt of turn.reply.options) {
            ...
            expect(
              accepted,
              `m23 ${l.id} ${s.id}/${turn.id}: option "${opt.text}" mirrors the NPC line but is marked wrong`,
            ).toBe(true);
          }
        }
      }
      ...
      for (const s of l.steps) {
        ...
        for (const turn of s.turns) {
          expect(
            ...
            `m23 ${l.id} ${s.id}/${turn.id} goal too wordy`,
  ...
  it("cued recall NEVER precedes a printed first voicing — across m1→m23 (R3/§13.9)", () => {
          ...
          if (s.cue === "recall") {
            if (counting) {
              ...
              expect(
                ...
                `m23 ${l.id} ${s.id}: recall of «${s.targetPhrase}» before any printed voicing`,
              ).toBe(true);
            }
          } else {
    ...
    walk(LESSONS, true);
    expect(m23Recalls).toBeGreaterThanOrEqual(8);
  });

  // ── Bespoke m23 pins (this module's own speech-safety design, m23.ts
  //    header decision 7) ──

  it("no `speaking` target differs from another `speaking` target ONLY by a numeral swap", () => {
    ...
    // Strip any numeral word from the phrase and compare stems — two
    // DIFFERENT full phrases sharing a stem means the numeral was the
    // only thing distinguishing them.
    const NUMERALS =
      /\b(un|une|deux|trois|quatre|cinq|six|sept|huit|neuf|dix|onze|douze)\b/gi;
    const stemOf = (t: string) => t.replace(NUMERALS, "•").trim();
  ...
  it("`heures` carries the SAME homophoneKey as `heure` (decision 1: the wiring that actually functions)", () => {
    const heure = FR_M23_ATOMS.find((a) => a.surface === "heure");
    const heures = FR_M23_ATOMS.find((a) => a.surface === "heures");
    expect(heure?.homophoneKey).toBeTruthy();
    expect(heures?.homophoneKey).toBeTruthy();
    expect(heures?.homophoneKey).toBe(heure?.homophoneKey);
  });

  it("exactly 2 new atoms this module — heure, heures", () => {
    expect(FR_M23_ATOMS.map((a) => a.surface).sort()).toEqual(["heure", "heures"]);
  });

  it("at most 3 graded non-recall `speaking` targets for the whole module (policy pin)", () => {
    let printedCount = 0;
    for (const l of LESSONS) {
      for (const s of l.steps) {
    ...
    expect(printedCount).toBeLessThanOrEqual(3);
  });

  it('no "qui" of any kind (question-word usage) appears anywhere in the module', () => {
    const bad: string[] = [];
    for (const l of LESSONS) {
      for (const s of l.steps) {
  ...
  it("never asks the learner to PRODUCE «vous», «qui», or verb-subject inversion (decision 6)", () => {
    for (const l of LESSONS) {
      for (const s of l.steps) {
        if (s.type === "dialogue_sim") {
          for (const t of s.turns) {
            if (t.reply.mode !== "choice") continue;
            for (const opt of t.reply.options) {
              if (opt.id !== t.reply.correctOptionId) continue;
              expect(/\bvous\b/i.test(opt.text)).toBe(false);
              expect(/-il\b|-elle\b/i.test(opt.text)).toBe(false);
            }
          }
        }
        if (s.type === "speaking") {
          expect(/\bvous\b/i.test(s.targetPhrase)).toBe(false);
          expect(/-il\b|-elle\b/i.test(s.targetPhrase)).toBe(false);
        }
        if (s.type === "build_sentence") {
          expect(/\bvous\b/i.test(s.targetSentence)).toBe(false);
          expect(/-il\b|-elle\b/i.test(s.targetSentence)).toBe(false);
        }
      }
    }
  });
