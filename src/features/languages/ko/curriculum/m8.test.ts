/**
 * KO M8 curriculum guard.
 *
 * Headline guard: every M8 pathway node declared in the course mock
 * (`ko-m8-1` … `ko-m8-8`) must resolve to real lesson content. If a future
 * edit declares them in the pathway without content (or vice versa) this
 * test fails loud.
 */
import { describe, it, expect } from "vitest";
import { KO_M8_LESSONS } from "./m8";
import { getMockCourse } from "@/shared/domain/mockCourse";
import { getMockLessonContent } from "@/features/lesson/data/mockLessons";

describe("KO M8 curriculum", () => {
  it("ships 8 lessons, all tagged ko / m8", () => {
    expect(KO_M8_LESSONS.length).toBe(8);
    expect(KO_M8_LESSONS.every((l) => l.moduleId === "m8")).toBe(true);
    expect(KO_M8_LESSONS.every((l) => l.languageId === "ko")).toBe(true);
  });

  it("lesson ids are unique", () => {
    const ids = new Set(KO_M8_LESSONS.map((l) => l.id));
    expect(ids.size).toBe(KO_M8_LESSONS.length);
  });
  ...
  it("every step id within a lesson is unique", () => {
    for (const lesson of KO_M8_LESSONS) {
