 ...
 *
 * Headline guard: every M2 pathway node declared in the course mock
 * (`es-m2-1` … `es-m2-8`) must resolve to real lesson content. If a future
...
import { ES_M2_ATOMS, ES_M2_LESSONS } from "./m2";
...
describe("ES M2 curriculum", () => {
  it("ships 8 lessons, all tagged es / m2 / mock-1", () => {
    expect(ES_M2_LESSONS.length).toBe(8);
    expect(ES_M2_LESSONS.every((l) => l.moduleId === "m2")).toBe(true);
    expect(ES_M2_LESSONS.every((l) => l.languageId === "es")).toBe(true);
    expect(ES_M2_LESSONS.every((l) => l.courseId === "mock-1")).toBe(true);
    expect(ES_M2_LESSONS.map((l) => l.id)).toEqual([
      "es-m2-1",
      "es-m2-2",
      "es-m2-3",
      "es-m2-4",
  ...
  it("lesson ids are unique", () => {
    const ids = new Set(ES_M2_LESSONS.map((l) => l.id));
    expect(ids.size).toBe(ES_M2_LESSONS.length);
  });
  ...
  it("every M2 pathway node resolves to lesson content", () => {
    ...
    expect(m2?.lessons.length ?? 0).toBe(8);
  ...
  it("every step id within a lesson is unique", () => {
    for (const lesson of ES_M2_LESSONS) {
  ...
  it("the mastery test contains graded steps only", () => {
    const mastery = ES_M2_LESSONS[7];
    expect(mastery.steps.every((s) => isGradedStep(s))).toBe(true);
  });

  it("every M2 atom surface literally appears in M2 steps", () => {
    const corpus = ES_M2_LESSONS.map((l) => JSON.stringify(l.steps)).join("\n");
    for (const atom of ES_M2_ATOMS) {
      expect(
        corpus.includes(atom.surface),
        `atom surface '${atom.surface}' never appears in M2 steps`,
      ).toBe(true);
    }
  });
});
