/**
 * ES M10 curriculum guard.
 *
 * Headline guard: every M10 pathway node declared in the course mock
 * (`es-m10-1` … `es-m10-8`) must resolve to real lesson content. If a
...
// "./m10" as the entry, m10 would sit suspended mid-import-cycle while
// later modules (whose match grids reference m10 surfaces) evaluate —
// and matchPairs throws on unregistered surfaces.
import "./index";
import { ES_M10_ATOMS, ES_M10_LESSONS } from "./m10";
...
describe("ES M10 curriculum", () => {
  it("ships 8 lessons, all tagged es / m10 / mock-1", () => {
    expect(ES_M10_LESSONS.length).toBe(8);
    expect(ES_M10_LESSONS.every((l) => l.moduleId === "m10")).toBe(true);
    expect(ES_M10_LESSONS.every((l) => l.languageId === "es")).toBe(true);
    expect(ES_M10_LESSONS.every((l) => l.courseId === "mock-1")).toBe(true);
    expect(ES_M10_LESSONS.map((l) => l.id)).toEqual([
      "es-m10-1",
      "es-m10-2",
      "es-m10-3",
      "es-m10-4",
    ...
    expect(ES_M10_LESSONS[7].title).toBe("M10 Mastery Test");
  });

  it("lesson ids are unique", () => {
    const ids = new Set(ES_M10_LESSONS.map((l) => l.id));
    expect(ids.size).toBe(ES_M10_LESSONS.length);
  });
  ...
  it("every M10 pathway node resolves to lesson content", () => {
    ...
    expect(m10?.lessons.length ?? 0).toBe(8);
    for (const lesson of m10!.lessons) {
  ...
  it("every step id within a lesson is unique", () => {
    for (const lesson of ES_M10_LESSONS) {
  ...
  it("the mastery test contains graded steps only", () => {
    const mastery = ES_M10_LESSONS[7];
    expect(mastery.steps.every((s) => isGradedStep(s))).toBe(true);
  });

  it("every M10 atom surface literally appears in M10 steps", () => {
    const corpus = ES_M10_LESSONS.map((l) => JSON.stringify(l.steps)).join("\n");
    for (const atom of ES_M10_ATOMS) {
      expect(
        corpus.includes(atom.surface),
        `atom surface '${atom.surface}' never appears in M10 steps`,
      ).toBe(true);
    }
  });
});
...
// m10 predates the bar (hand-authored July wave); measured debt is pinned
...
registerEsModuleBarGuards({
  ...
  debt: { unknownTokens: 60, nonIntroDebuts: 6, fullSentenceMcqs: 17, productionFramedMcqs: 6, imageMcqReuse: 1, particleClozeOutOfModule: 1, translateShare: 0.266 },
});
