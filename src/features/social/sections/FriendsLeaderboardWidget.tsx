    ...
    <Card padding="none" className="overflow-hidden">
      ...
      <ul className="divide-y divide-border">
        ...
        <li className="flex items-center gap-2.5 bg-surface px-3 py-2.5">
          ...
          <p className="text-sm font-bold text-text-primary">{top.xp.toLocaleString()} XP</p>
        </li>

        {/* Me */}
        {myRow ? (
          <li className="flex items-center gap-2.5 bg-accent-muted px-3 py-2.5">
            <span className="flex h-7 w-7 items-center justify-center rounded-full bg-accent text-on-accent shadow-sm text-[11px] font-bold">
              #{myRow.rank}
            </span>
            <div className="min-w-0 flex-1">
              <p className="text-[10px] font-semibold uppercase tracking-wider text-text-muted">
            ...
            <p className="text-sm font-bold text-text-primary">{myRow.xp.toLocaleString()} XP</p>
          </li>
        ) : null}
      </ul>
      ...
      <div className="border-t border-border bg-surface-muted px-3 py-2 text-[11px]">
        {xpToOvertake && aboveMe ? (
          <p className="text-text-secondary">
            <Icon name="arrowRight" size={11} aria-hidden className="mr-1 inline-block align-middle" />
      ...
      <div className="flex items-center gap-2 border-t border-border bg-surface px-3 py-2 text-[11px]">
        ...
        <span className="text-text-muted">
          {t("social.streak.vsMedian", "vs. friend median {{n}}d", { n: streaks.data.friendMedian })}
        </span>
        <span
          className={cn(
