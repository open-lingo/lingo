...
describe("granular hooks", () => {
  ...
  it("useFriends respects instant: true", () => {
    const { result } = renderHook(() => useFriends({ instant: true }));
    expect(result.current.isLoading).toBe(false);
    expect(result.current.data).not.toBeNull();
    expect(result.current.data!.length).toBeGreaterThan(0);
  });
  ...
  it("useActivityFeed yields ActivityItems with reactions", async () => {
    const { result } = renderHook(() => useActivityFeed({ instant: true }));
    expect(result.current.data?.[0]?.reactions).toBeDefined();
  });

  it("useInviteOffer exposes the lingot + ad-free reward", () => {
    const { result } = renderHook(() => useInviteOffer({ instant: true }));
  ...
  it("useLeagueSpotlight derives the day-over-day rank delta", () => {
    const { result } = renderHook(() => useLeagueSpotlight({ instant: true }));
