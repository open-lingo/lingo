...
export function ToastContainer({
  ...
  /** Sidebar nav mode: wherever the rail is showing, the utility pill docks
   *  bottom-right, so toasts stack from the top-right instead of covering it.
   *  Mobile — and a portrait tablet, which has no rail — keeps the bottom
   *  stack. Gated on the same `landscapeLg` screen as the rail itself
   *  (SidebarNav.tsx), so the two can never disagree about whether the rail
   *  is on screen. */
  topRightOnSidebar?: boolean;
} = {}) {
  ...
  return (
    <div
      className={`pointer-events-none fixed ${bottomOffsetClass ?? "bottom-[var(--funding-meter-height,4.5rem)]"} left-4 right-4 z-[60] flex flex-col gap-2 sm:left-auto sm:right-4 sm:max-w-sm ${
        topRightOnSidebar ? "landscapeLg:bottom-auto landscapeLg:top-4" : ""
      }`}
      aria-live="polite"
      aria-atomic="true"
    >
