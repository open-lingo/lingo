...
w(`2. Write \`specs/pt-m<mod>-l<n>.yaml\` — with \`spine: <lesson-id>\`, ~15-25 lines (format below).`);
w(`3. \`node scripts/author/pt/from-spec.mjs specs/pt-m<mod>-l<n>.yaml\``);
w(`4. \`bash scripts/author/pt/check.sh <n> [<module-lesson-count>] [--module m<mod>]\` (module defaults to m1)`);
w(`5. Commit the two generated files + the spec.`);
w();
w(`## Spine-first specs (lane PTTOOL4)`);
w(`Add \`spine: <lesson-id>\` (e.g. \`pt-m2-1\`, from \`spine/pt-spine.yaml\`) and the spec need only supply`);
w(`\`sentences:\` + \`dialogue:\` — title/grammar/info/infoTitle/words/recall/antiPattern/contrast/win/scene`);
w(`all inherit; an explicit field still overrides the spine's (the generator prints "overrides spine: <field>").`);
w(`\`winOverride: <reason>\` is required when a spec's own \`win:\` differs from its spine lesson's, or`);
w(`check.sh's payoff-rule FAILs. \`allowExtra: [<word>]\` + \`reason:\` is the one-off, named exception to`);
w(`the \`allow:\` closed set (e.g. a spine word not yet taught in THIS lesson) — never a silent second entry.`);
w();
