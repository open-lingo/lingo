...
REGRAS: turnos 1 e 3 são de escolha com exatamente 3 opções; o turno 2 é `mode: build`. Num turno de escolha a opção certa tem de RESPONDER à fala do personagem e ao goal; as duas erradas têm de ser gramaticais, do mesmo tamanho, e claramente NÃO responder (pessoa errada, coisa errada, ou um non sequitur que um iniciante reconheça). No turno build, `answer` sem pontuação final e respondendo à fala; `tiles` = as palavras da resposta + 2 peças erradas. `gloss` e `goal` em inglês; `goal` com no máximo 8 palavras. Soe como gente conversando, não como exercício.
