...
- If the lesson has a two-form pair, add `contrastSet: [{{ set: [x, y], why: "one sentence, ≥ 25 chars, in your own words" }}]`. If the lesson ALSO teaches an article/determiner/pronoun pair (um/uma, o/a, meu/minha…), put ONE `cloze:` on that pair too (not one per member), and give every such cloze sentence its own `why: "…"` field (one sentence saying why that form is right there). Most clozes must blank the lesson's own grammar point, not articles or pronouns.
