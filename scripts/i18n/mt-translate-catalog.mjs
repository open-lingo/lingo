...
CONVENTIONS (docs/ko-content-conventions-2026-09-10.md — follow exactly):
  ...
  - PARTICLES に vs で: に marking destination/goal of motion (えきに いく) → 에 (역에 간다); で marking means/instrument (でんしゃで) → 로/으로 (전철로); never 로 for a destination.
  - VOCAB GLOSS — すきだ/きらいだ: render as 좋다/싫다 with 이/가 on the object (것이 좋다), never 좋아하다/싫어하다 + 을/를, unless the JA itself uses a transitive verb.
  - DIGIT+COUNTER SPACING: fuse an Arabic-numeral digit directly onto its counter — NO space — for 시/분/월/일/살/세/개/장/명/번/층/과 and similar counters (9시부터 5시까지, 3월, 5개, 2명, 25세); a native-Korean spelled-out numeral word still takes a space before its counter (두 장, 세 명 stay spaced). Confirmed drift class: the m16 draft wrote "9 시부터 5 시까지" (spaced) — 39 of 103 review fixes were this one pattern; a second m17 miss was 세 (age counter) not fused — 25 fixes.
  - FIXED WORD CHOICES (m17 review, confirmed corpus decisions): あに/あね → 형/누나 (this course's atom glosses fix the speaker as male — NEVER 오빠/언니); おとうと → 남동생; どなたですか → 누구세요?; 대학 (never 대학교); 차 (never 자동차).
  - REGISTER — FUSED 합니다체 ENDINGS: in a plain-register module, NO 합니다체 ending is allowed in ANY shape, including a -ㅂ니다/-습니다 fused onto the stem's final syllable (엽니다, 삽니다, 놉니다, 갑니다, etc.) — these must still render as the plain form (논다, 산다, 간다), EXCEPT inside a ください-request rendering (…해 주세요 stays as-is) or a prohibition (…하지 마세요/…해서는 안 됩니다), which keep their own required register.
  - STATIVE-ACHIEVEMENT ている: しる/すむ/もつ-class stative-achievement verbs in ている ALWAYS render as -고 있다/-고 있어요 (알고 있다, 살고 있다, 가지고 있다), never flattened to a bare present (안다, 산다) — the ongoing-state meaning must stay visible in the gloss.
  ...
  - SENTENCE FORMS: a full sentence is never left in the bare dictionary form (-다 stem); conjugate it to the register the item's register rule selects (해요체 → 놀아요, 해체 → 놀아, plain written → 논다, 합니다체 → 놉니다) — e.g. EN "I play with a friend on Thursday" (plain) → 목요일에 친구와 논다, not 놀다. This rule binds "ja-gloss" and "build-prompt" kinds specifically: despite the word "gloss" in "ja-gloss", both are FULL SENTENCES to conjugate, NOT dictionary-form citations like "vocabulary gloss (atom)"/"story-gloss" correctly are — don't let the shared word "gloss" pull a sentence back toward citation form.
  ...
  - VERB SENSE PINS: わかる with a person subject → 알아듣다 family (알아들어요/알아듣는다), with a が-marked theme subject → 이해가 (안) 되다 (그 이야기가 이해가 안 된다), never 알다/이해하다; すし → 초밥; すきじゃない → 좋지 않다 (keep the story's own すきだ→좋다 word, never 싫다).
  - QUOTATION と: 〜と おもう → 〜다고/〜ㄴ다고 생각하다 with the quoted clause in plain form (미카는 온다고 생각하나?); 〜と いう → 〜다고 하다; Template A question ending is -나? (not -니?); volitional inside a quote keeps -겠다.
