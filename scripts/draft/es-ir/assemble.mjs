...
export function makeAssembler({ frame, pool, moduleId, tense = "preterite" }) {
  ...
  const S = {
    ...
    speakLit: (id, o) => {
      requireLit(id, o, ["es", "en", "atoms"]);
      return `    speaking(${q(id)}, ${q(lower1(bare(o.es)))}, ${q(bare(o.en))}, [${o.atoms.map(q).join(", ")}]),`;
    },

    listenCompLit: (id, o) => {
      requireLit(id, o, ["es", "en", "atoms", "distractorsEn"]);
    ...
    clozeLit: (id, o) => {
      requireLit(id, o, ["es", "en", "blank", "options", "atoms"]);
      const w = words(o.es);
